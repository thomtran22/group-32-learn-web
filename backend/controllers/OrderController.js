...
const acceptOrder = async (req, res) => {
  try {
    ...
    const order = await Order.findOneAndUpdate(
      ...
      {
        shipperId: shipperId,
        status: "Shipping", // Cập nhật trạng thái thành Shipping (Khớp enum Model)
      },
      { new: true }
    );
    ...
    if (!order) {
      return res.status(400).json({
        message: "Đơn hàng này đã có người nhận hoặc không tồn tại.",
      });
    }
...
// 3. API Thống kê (Cập nhật lại theo status và totalPrice)
const getShipperStats = async (req, res) => {
  try {
    ...
    const activeDeliveryCount = await Order.countDocuments({
      shipperId: shipperId,
      status: "Shipping", // Khớp enum
    });

    // Tính tổng thu nhập (Chỉ tính đơn đã giao xong - Delivered)
    // Lưu ý: Đây là tính tổng giá trị đơn hàng, nếu bạn tính lương shipper thì logic khác nhé
